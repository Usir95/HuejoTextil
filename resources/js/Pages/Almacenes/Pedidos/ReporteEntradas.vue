...
<script setup>
import { ref, inject, defineProps, onMounted, computed, watch } from 'vue'
...
const props = defineProps({
    ...
    num_tarjeta: [String, Number, null],
})
...
/**
 * Normaliza tipos para que el v-model haga match con options {value,label}
 * Si tus options usan number, cambia a Number() en vez de String().
 */
const normalizeId = (v) => {
...
/* ========================== Cargar Tarjetas ========================== */
const CargarTarjetas = async (clienteId) => {
    if (!clienteId) {
...
/* ========================== Watch cliente ========================== */
watch(() => form.cliente_id, async (nuevoClienteId, anteriorClienteId) => {
    if (anteriorClienteId && nuevoClienteId && anteriorClienteId !== nuevoClienteId) {
        form.num_tarjeta = ''
    }
    await CargarTarjetas(nuevoClienteId)
})
...
const Buscar = async () => {
    ...
    try {
        const { data } = await axios.post(route('HistoricoEntradas.FiltrarEntradas'), {
            ...
            num_tarjeta: form.num_tarjeta || null,
        })
...
/* ========================== Auto-carga por URL ========================== */
onMounted(async () => {
    const clienteId = normalizeId(props.cliente_id)
    const tarjetaId = normalizeId(props.num_tarjeta)

    if (clienteId) form.cliente_id = clienteId
    if (clienteId) {
        await CargarTarjetas(clienteId)
    }

    if (tarjetaId) form.num_tarjeta = tarjetaId

    if (clienteId || tarjetaId) {
        await Buscar()
    }
})
...
const Encabezado = computed(() => ({
    cliente:
        safe(HistoricoEntradas.value?.[0]?.cliente?.nombre) ||
        safe(props.Clientes?.find?.(c => String(c.value) === String(form.cliente_id))?.label) ||
        '',
