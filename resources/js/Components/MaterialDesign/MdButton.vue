...
<template>
  <button
    ...
    @click="$emit('click')"
    :class="[
      'px-4 py-1 rounded-xl font-semibold transition-colors duration-150 inline-flex items-center justify-center gap-2',
      disabled || loading ? 'opacity-50 cursor-not-allowed' : 'hover:shadow-md',
      variant === 'primary' && 'text-white bg-[var(--color-primary)] hover:bg-[var(--color-primary-hover)] dark:bg-[var(--color-primary-light)]',
      variant === 'secondary' && 'text-white bg-[var(--color-secondary)] hover:brightness-110',
